...
def get_post(slug, current_path=""):
    ...
    tags = [A(f"#{tag}", href=f"/tags/{tag}") for tag in post['tags']]
    return root_layout(Div(Titled(post['title'], Div(post.content, cls="marked"), P(*tags))), current_path)

def get_posts_by_tag(tag, current_path=""):
    posts = get_posts()
    ...
    return root_layout(Titled(f"Posts tagged with '{tag}'", Ul(*post_items)), current_path)
...
def get_tag_list(current_path=""):
    tags = get_all_tags()
    tag_items = [Li(A(f"#{tag}", href=f"/tags/{tag}")) for tag in tags]
    return root_layout(Titled("All Tags", Ul(*tag_items)), current_path)
