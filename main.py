...
def get():
    ...
    post_items = [Div(H2(A(post['title'], href=f"/posts/{post['filename']}", **{"class": "text-2xl font-bold hover:text-blue-700"})),
                      ...
                      P("Tags: " + ", ".join(str(A(tag, href=f"/tag/{tag}", **{"class": "text-gray-500 hover:text-blue-700"})) for tag in post['tags']), **{"class": "text-gray-500"})) for post in posts]
...
def tag_detail(tag: str):
    ...
    post_items = [Div(H2(A(post['title'], href=f"/posts/{post['filename']}", **{"class": "text-2xl font-bold hover:text-blue-700"})),
                      P(f"Date: {post['date']}", **{"class": "text-gray-600"}),
                      P("Tags: " + ", ".join(str(A(tag, href=f"/tag/{tag}", **{"class": "text-gray-500 hover:text-blue-700"})) for tag in post['tags']), **{"class": "text-gray-500"})) for post in tagged_posts]
